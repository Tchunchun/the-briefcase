...
- `briefcase setup` CLI command with interactive backend selection (default: `local`)
...
- `briefcase setup --backend notion` — one command to provision, seed, and configure
- `briefcase sync local` — generates local markdown from Notion for git audit trail
- `--dry-run` flag for safe preview

### Phase 3 — Template Management
- `briefcase sync templates` — pulls updated templates from Notion back to `template/`
...
| T-004 | Factory + `briefcase setup` CLI | Done |
...
| T-008 | `briefcase setup --backend notion` CLI extension | Done |
| T-009 | `briefcase sync local` command + sync logic | Done |
| T-010 | `briefcase sync templates` command | Done |
...
3. Run `briefcase setup` to choose backend
4. For Notion: provide API token + parent page ID when prompted

## Rollback

- For local backend: delete `_project/storage.yaml` and re-run `briefcase setup`
- For Notion backend: Notion databases remain in place; switch back to local via `briefcase setup --backend local`
