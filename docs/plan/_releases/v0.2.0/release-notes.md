...
**New command: `agent sync notion`** — push local markdown to Notion (reverse of `agent sync local`).
...
| CLI sync | `src/cli/commands/sync.py` — added `agent sync notion` |
...
- Sync is manual (`agent sync local` / `agent sync notion`) — automation via hooks is a future enhancement
