# Delivery Manager Orchestrated Mode (v2)
...
- **Latency / response time:** Delivery-manager routing/delegation overhead under 30 seconds per transition, excluding implementation/review execution time
- **Availability / reliability:** If subagent delegation fails, delivery-manager records a blocked transition with explicit recovery path
- **Cost constraints:** No required paid tooling or external orchestration service
- **Compliance / data residency:** No new data classes; use existing repository artifacts only
- **Other constraints:** Must reuse existing framework and role skills; no bespoke orchestration framework
...
- This brief is v2 of delivery-manager adoption, focused on single-interface orchestration.
- Constraint from user request: do not hand-roll a new framework; compose existing role skills and playbook rules.
