...
- `docs/plan/_inbox.md`
- `docs/plan/{feature-name}/brief.md`

Guideline file: `_agent_guideline/ideation-agent-guideline.md`

Do not use this agent for: coding, task breakdown, backlog execution, release documentation, or writing to `src/` or `tests/`.
...
1. Ideation agent captures or scopes the work → produces `brief.md`.
2. Implementation agent breaks it down and builds it → produces `tasks.md`, code, tests.
3. Review agent checks the result against the requirement.
4. Implementation agent ships accepted work and writes release notes.
...
| **1 Plan** | Idea promoted | Ideation agent creates `{feature}/brief.md` and makes it implementation-ready. Mark inbox item `[-> planned]`. |
...
| `docs/plan/{feature}/brief.md` | Ideation agent | Implementation and review agents: read-only |
...
| `_project/` | Set during setup | Any agent may read; changes require explicit instruction |
...
- **Agent ownership matters.** Ideation owns scope, implementation owns delivery, review owns acceptance.
